...
export default async function Categories() {
  ...
  return (
    <section className="w-full px-4 sm:px-6 mt-6">
          ...
          categories?.data.map((category) => (
            ...
            >
              ...
              <div className="flex flex-col items-center">
                ...
                <p className="text-xs text-[#6B7280]">{category.count || 2} Products</p>
              </div>
            </Link>
          ))}
      </div>

      {/* Desktop & Tablet: Balanced Responsive Grid (Max 7 Columns) */}
      <div className="hidden md:grid grid-cols-4 lg:grid-cols-6 xl:grid-cols-7 gap-6">
        {categories?.data.map((category) => (
          <Link
            key={category._id}
          ...
          >
            ...
            <div className="flex flex-col items-center text-center w-full">
              <p className="text-sm font-medium text-[#1E1E1E] truncate max-w-full">{category.name}</p>
              <p className="text-xs text-[#6B7280]">{category.count || 0} Products</p>
            </div>
          </Link>
        ))}
      </div>
